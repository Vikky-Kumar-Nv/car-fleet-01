...
import { getMultipartAuthHeaders } from '../helpers/auth.helper';
...
describe('File Upload Integration Tests', () => {
  ...
  describe('Vehicle File Upload', () => {
    it('should handle vehicle creation with photo upload', async () => {
    ...
    it('should handle vehicle creation with document upload', async () => {
      const response = await request(app)
        .post('/api/vehicles')
        .set(getMultipartAuthHeaders(adminUserId, 'admin'))
    ...
    it('should handle vehicle update with file upload', async () => {
  ...
  describe('Driver File Upload', () => {
    it('should handle driver creation with photo upload', async () => {
      const response = await request(app)
        .post('/api/drivers')
        .set(getMultipartAuthHeaders(adminUserId, 'admin'))
    ...
    it('should handle driver creation with license document upload', async () => {
      const response = await request(app)
        .post('/api/drivers')
        .set(getMultipartAuthHeaders(adminUserId, 'admin'))
    ...
    it('should handle driver creation with police verification document upload', async () => {
      const response = await request(app)
        .post('/api/drivers')
        .set(getMultipartAuthHeaders(adminUserId, 'admin'))
    ...
    it('should handle driver creation with multiple file uploads', async () => {
      const response = await request(app)
        .post('/api/drivers')
        .set(getMultipartAuthHeaders(adminUserId, 'admin'))
    ...
    it('should handle driver update with file upload', async () => {
  ...
  describe('File Upload Error Handling', () => {
    it('should handle requests without files gracefully', async () => {
      const response = await request(app)
        .post('/api/vehicles')
        .set(getMultipartAuthHeaders(adminUserId, 'admin'))
    ...
    it('should maintain authentication on multipart requests', async () => {
      const response = await request(app)
        .post('/api/drivers')
    ...
    it('should maintain authorization on multipart requests', async () => {
      const customerUserId = '507f1f77bcf86cd799439015';
      
      const response = await request(app)
        .post('/api/drivers')
        .set(getMultipartAuthHeaders(customerUserId, 'customer'))
  ...
  describe('Upload Middleware Configuration', () => {
    it('should accept all configured field names', async () => {
